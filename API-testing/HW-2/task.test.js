...
beforeAll(async ()=>{
	...
	const tokenResponse = await UserController.getToken(users.userFirst.username, users.userFirst.password)
...
afterAll(async () => {
	const response = await BookController.removeAllBooks(
		users.userFirst.userId
	);
